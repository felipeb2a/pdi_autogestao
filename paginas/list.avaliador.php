<?php
    ...
    $resultAvaliador = listAvaliador($_POST['func_select']);
							...
							<table class="table table-bordered">
								<tbody>
									<tr class="text-center thead-dark">
			                   		...
			                   		<?php
			                  			//loop funcionario
			                   			while($funcionario = mysqli_fetch_array($resultAvaliador)){
									 		echo '<tr class="text-center">';
								    			echo '<th scope="row">'.$funcionario['funcionario'].'</th>';
								    			echo '<th scope="row">'.$funcionario['avaliador'].'</th>';
								    			//button remover
								    			echo '<th scope="row">';
									    			echo '<form action="../functions/avaliador.remove.php" method="post" style="margin-top: 30px;">';
